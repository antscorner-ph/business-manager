...
import { useProducts, type Product, type StockFilter } from "@/hooks/useProducts";
...
/** Renders a colored badge for a product's remaining inventory using its own low-stock threshold. */
function InventoryBadge({ qty, lowStock }: { qty: number | null; lowStock: number | null }) {
  if (qty === null) {
    return <Badge variant="outline">No stock data</Badge>;
  }
  const threshold = lowStock ?? 0;
  if (qty <= 0) {
    return <Badge variant="destructive">Out of stock</Badge>;
  }
  if (qty <= threshold) {
    return <Badge variant="secondary">Low · {qty} on hand</Badge>;
  }
  return <Badge variant="outline">{qty} on hand</Badge>;
}
...
export function POProductSearch({ onAdd }: POProductSearchProps) {
  ...
  const { products, loading } = useProducts(debounced, stockFilter);
              ...
              {products.map((product) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <InventoryBadge qty={product.qty} lowStock={product.low_stock} />
                  </div>
